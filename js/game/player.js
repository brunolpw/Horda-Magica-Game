...
function updatePassivePlayerAbilities() {
    ...
    enemies.forEach(enemy => {
        ...
        if (data.burnTimer > 0 && data.burnTimer % 120 === 0) { // Queimadura
            let damage = 10;
            damage *= getWeaknessMultiplier('fire', data.type);
            data.hp -= damage;
            createFloatingText(Math.floor(damage), mesh.position.clone().setY(data.modelHeight || 1.5), '#ff4500');
            data.hitTimer = 5;
        }
    });
}
