...
let debugWindow; // NOVO: Referência para a janela de debug
// --- Variáveis de UI ---
...
function setupUIElements() {
    ...
    // NOVO: Adiciona o listener para o novo botão de debug
    const startDebugButton = document.getElementById('start-debug-button');
    if (startDebugButton) {
        startDebugButton.onclick = () => handleStartGameClick(true); // O 'true' ativa o modo debug
    }

    createDebugWindow(); // NOVO: Cria a janela de debug (inicialmente oculta)
}
...
function handleStartGameClick(isDebugging = false) { // NOVO: Parâmetro para modo debug
    ...
    startGame(playerNameInput, isDebugging); // NOVO: Passa a flag de debug para o startGame
}
...
function showLevelUpOptions() {
    ...
    let chosenUpgrades = [];
    // AJUSTE: No modo debug, mostra todas as opções de upgrade
    if (isDebugMode) {
...
function createDebugWindow() {
    ...
    debugWindow.className = 'hidden p-4 bg-gray-900 bg-opacity-75 rounded-lg shadow-lg text-white text-sm';
    debugWindow.style.position = 'absolute';
    debugWindow.style.top = '150px';
    debugWindow.style.right = '10px';
    debugWindow.style.border = '1px solid #4a5568';

    const shortcuts = {
        '1': 'Goblin', '2': 'Orc', '3': 'Troll',
        '4': 'Necromancer', '5': 'Skeleton Warrior', '6': 'Fire Elemental',
        '7': 'Ice Elemental', '8': 'Lightning Elemental', '9': 'Ghost',
        'L': 'Ganhar EXP'
    };

    let content = '<h3 class="text-lg font-bold mb-2 text-yellow-400">Atalhos de Debug</h3><ul>';
    for (const key in shortcuts) {
        content += `<li class="mb-1"><span class="font-bold text-yellow-300">${key}:</span> ${shortcuts[key]}</li>`;
    }
    content += '</ul>';

    debugWindow.innerHTML = content;
    document.body.appendChild(debugWindow);
}

function showDebugWindow() {
    if (debugWindow) debugWindow.classList.remove('hidden');
}

function hideDebugWindow() {
    if (debugWindow) debugWindow.classList.add('hidden');
}
